...
class UnifiedApplicationModel(BaseModel):
    ...
    status: str = Field(
        default="saved",
        pattern="^(saved|pending|pending_verification|under_review|review_application|approved|rejected|completed|verification_successful|verification_rejected|confirmed_application|update_application|approved_application|update_rejected|final_submitted|replaced|payment_pending|payment_verified)$"
    )
    ...
    # QR code data (REMOVED - Razorpay only)
    # qr_code_data: Optional[str] = None  # ❌ REMOVED
    # qr_image_url: Optional[str] = None   # ❌ REMOVED
    # upi_text: Optional[str] = None       # ❌ REMOVED
