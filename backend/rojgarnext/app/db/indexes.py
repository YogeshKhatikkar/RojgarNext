# app/db/indexes.py - UPDATED with unified collections
...
async def create_index_safely(collection, keys, **kwargs):
    ...
    try:
        ...
        if "already exists" in str(e) or "IndexKeySpecsConflict" in str(e) or "IndexOptionsConflict" in str(e):
...
async def setup_all_indexes() -> Dict[str, Any]:
    ...
    await create_index_safely(db.profile, "category")  # NEW
    await create_index_safely(db.profile, "disability.is_disabled")  # NEW
    await create_index_safely(db.profile, "disability.category")  # NEW
