...
class _PaymentScreenState extends State<PaymentScreen> with SingleTickerProviderStateMixin {
  ...
  void _handleExpiration() {
    ...
    _showMessage("⚠️ Payment session has expired. Please try again.", isError: true);
  ...
  Future<void> _createRazorpayOrder() async {
    ...
    try {
      ...
      if (widget.paymentType == PaymentType.job) {
        ...
        if (widget.jobTitle != null) requestData["job_title"] = widget.jobTitle;
        if (widget.organization != null) requestData["organization"] = widget.organization;
      } else {
        ...
        if (widget.formData != null) requestData["form_data"] = widget.formData;
        if (widget.userEmail != null) requestData["user_email"] = widget.userEmail;
        if (widget.userName != null) requestData["user_name"] = widget.userName;
      }
      ...
      if (response.data['success'] == true) {
        ...
        // ✅ USE THE PLATFORM-AWARE SERVICE
        await RazorpayService.instance.initiatePayment(
          ...
          onExternalWallet: () {
            ...
            _showMessage("External wallet selected. Please complete payment in the app.");
          },
        );
      } else {
        final errorMsg = response.data['message'] ?? "Failed to create payment order";
      ...
      if (mounted && !_isDisposed) {
        _showMessage("Error: ${e.toString().replaceAll('Exception:', '').trim()}", isError: true);
        setState(() => _isRazorpayLoading = false);
      }
    }
  }
  ...
  void _handleRazorpaySuccess(Map<String, dynamic> response) {
    ...
    _razorpayOrderId = response['razorpay_order_id']?.toString() ?? _razorpayOrderId;
  ...
  Future<void> _verifyRazorpayPayment() async {
    try {
      ...
      if (response.data['payment_verified'] == true) {
        ...
        final errorMsg = response.data['message'] ?? "Payment verification failed.";
        _showMessage(errorMsg, isError: true);
        setState(() => _isRazorpayLoading = false);
      }
    } catch (e) {
      ...
      if (mounted && !_isDisposed) {
        _showMessage("Payment verification failed. Please contact support.", isError: true);
        setState(() => _isRazorpayLoading = false);
      }
    }
  }
  ...
  Widget build(BuildContext context) {
    ...
    final timeColor = _timeLeft.inMinutes < 2 && !isExpired ? Colors.red : Colors.blue;
    ...
    final bool canPay = !_isPaymentCompleted && !isExpired && !_isRazorpayLoading;
              ...
              if (!_isPaymentCompleted && !isExpired)
                SizedBox(
                  ...
                  child: ElevatedButton.icon(
                    ...
                    label: Text(
                      _isRazorpayLoading ? "Processing..." : 
                      kIsWeb ? "Pay with Razorpay (Web)" : "Pay with Razorpay",
